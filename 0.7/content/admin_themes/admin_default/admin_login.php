...
<p class="breadcrumbs">
    ...
    &raquo; <?php echo $admin->lang["admin_theme_login"]; ?>
</p>
        
<?php $admin->hotaru->showMessage(); ?>
        
<div class='main_inner'>
    <?php echo $admin->lang["admin_theme_login_instructions"]; ?>
    
    <form name='login_form' action='<?php echo BASEURL; ?>admin_index.php' method='post'>    
    <table>
        <tr>
        <td><?php echo $admin->lang["admin_theme_login_username"]; ?>:&nbsp; </td>
        <td><input type='text' size=30 name='username' value='<?php echo $username_check; ?>' /></td>
        </tr>
        <tr>
        <td><?php echo $admin->lang["admin_theme_login_password"]; ?>:&nbsp; </td>
        <td><input type='password' size=30 name='password' value='<?php echo $password_check; ?>' /></td>
        </tr>
        <tr>
        <td>&nbsp; </td>
        <td style='text-align:right;'><input type='submit' value='<?php echo $admin->lang['admin_theme_login_form_submit']; ?>' /></td>
        </tr>            
    </table>
    ...
    <form style="display: none;" name='forgot_password_form' action='<?php echo BASEURL; ?>admin_index.php' method='post'>    
        <?php echo $admin->lang['admin_theme_login_forgot_password_submit_instruct_1']; ?>
    <table>
        <tr>
        <td><?php echo $admin->lang["admin_theme_update_email"]; ?>&nbsp; </td>
        <td><input type='text' size=30 name='email' value='<?php echo $email_check; ?>' /></td>
        <td><input type='submit' value='<?php echo $admin->lang['admin_theme_login_forgot_password_submit']; ?>' /></td>
        </tr>            
    </table>
