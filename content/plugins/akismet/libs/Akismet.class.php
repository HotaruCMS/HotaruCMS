...
class Akismet
	 ...
	 */
	public function __construct($cage, $blogURL, $wordPressAPIKey) {
	    $this->cage = $cage;
		...
		$this->comment['user_agent'] = $this->cage->server->getRaw('HTTP_USER_AGENT');
		
		if(isset($this->cage->server->getRaw['HTTP_REFERER'])) {
			//$this->comment['referrer'] = $_SERVER['HTTP_REFERER'];
			$this->comment['referrer'] = $this->cage->server->getRaw('HTTP_REFERER');
		}
		...
		$this->comment['user_ip'] = $this->cage->server->getRaw('REMOTE_ADDR') != $this->cage->server->getRaw('SERVER_ADDR') ? $this->cage->server->getRaw('REMOTE_ADDR') : $this->cage->server->getRaw('HTTP_X_FORWARDED_FOR');
	}
