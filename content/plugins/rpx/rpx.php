...
class RPX extends PluginFunctions
{
    ...
    public function install_plugin()
    {
        if (!$this->db->column_exists('users', 'user_rpx')) {
            ...
            $sql = "ALTER TABLE " . TABLE_USERS . " ADD user_rpx TEXT NULL AFTER user_date";
    ...
    public function hotaru_header()
    {
        ...
        if ($this->hotaru->isPage('account') && ($this->account == 'basic')) {
            ...
            $sql = "UPDATE " . TABLE_USERS . " SET user_rpx = %s WHERE user_id = %d";
    ...
    public function users_register_post_add_user($vars)
    {
        ...
        // add the rpx profile info (serialized) into the users table
        $sql = "UPDATE " . TABLE_USERS . " SET user_rpx = %s WHERE user_id = %d";
        $this->db->query($this->db->prepare($sql, $rpx_profile, $last_insert_id));
    ...
    public function users_account_pre_password_user_only()
    {
        ...
        if (($this->account != 'basic') || $no_providers) {
            ...
            $output .= "href='https://japansoc-test-app.rpxnow.com/openid/v2/signin?token_url=" . $this->tokenUrl . "'>";
    ...
    public function users_register_error_check()
    {
        ...
        if ($result) {
            $this->includeLanguage();
            $rpx_profile = unserialize($result->user_rpx);
            $error_msg = $this->lang['rpx_users_register_exists_error_1'] . " " . $rpx_profile['providerName'] . ".<br/>";
            $error_msg .= $this->lang['rpx_users_register_exists_error_2'];
            
            if ($this->account != 'basic') {
                $error_msg .= "<br /> <br/>" . $this->lang['rpx_users_register_exists_error_3'];
            }
