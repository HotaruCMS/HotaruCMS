...
<style type="text/css">
    /* You should probably move this CSS into your link_bar.css file */
    .lb_vote_button { display: inline; }
    .vote_button_top { display: inline;  float: left; }
    .vote_button_bottom { display: inline; float: left; }
</style>

<!-- Vote Button -->
<div class='lb_vote_button'>
...
<?php if ($h->currentUser->loggedIn && !$h->vars['voted']) { ?>
    ...
    <div id='text_vote_<?php echo $h->post->id; ?>' class='vote_button_bottom'>
        <a href="#" onclick="vote( <?php echo $h->post->id; ?>, 10); return false;"><?php echo $h->lang["vote_button_vote"]; ?></a>
    </div>    
