...
class UserSignin
{
    ...
    public function navigation_users($h)
    {
        if ($h->currentUser->loggedIn) {
            ...
            if ($h->currentUser->getPermission('can_access_admin') == 'yes') {
                ...
                if ($h->pageName == 'admin') { $status = "class=\"active\""; } else { $status = ""; }
            ...
            { 
               ...
               if ($h->pageName == 'register') { $status = "class=\"active\""; } else { $status = ""; }
               echo "<li " . $status . "><a href='" . $h->url(array('page'=>'register')) . "'>" . $h->lang["user_signin_register"] . "</a></li>";
			   if (!$h->isPage('login')) {
                    echo "
					<li class=\"dropdown\">
						<a href=\"#\" class=\"dropdown-toggle\" data-toggle=\"dropdown\">" . $h->lang["user_signin_login"] . "<b class=\"caret\"></b></a>
						<ul class=\"dropdown-menu\">
							" . $h->pluginHook('user_signin_login_pre_login_form') . "
							<form name=\"login_form\" action=\"" . BASEURL ."\" method=\"post\">
								<div style=\"margin:5px 15px 0 15px;\">
									<label class=\"control-label\" for=\"inputUsername\">" . $h->lang["user_signin_login_form_submit_username"] . "</label>
									<div class=\"controls\">
										<input type=\"text\" name=\"username\" value=\"" . $username_check . "\">
									</div>
								</div>
								<div style=\"margin:5px 15px 0 15px;\">
									<label class=\"control-label\" for=\"inputPassword\">" . $h->lang["user_signin_login_form_submit_password"] . "</label>
									<div class=\"controls\">
										<input type=\"password\" name=\"password\" value=\"" . $password_check ."\">
									</div>
								</div>
								<div style=\"margin:5px 15px 0 15px;\">
									<div class=\"controls\">
										<label class=\"checkbox\">
											<input type=\"checkbox\" name=\"remember\" value=\"1\" " . $remember_check . " /> " . $h->lang["user_signin_login_form_submit_remember"] . "
										</label>
										<input type=\"hidden\" name=\"page\" value=\"login\">
										<input type=\"hidden\" name=\"return\" value=\"" . $return_check . "\">
										<input type=\"hidden\" name=\"csrf\" value=\"" . $h->csrfToken . "\" />
										<input type=\"submit\" class=\"btn btn-block btn-primary\" value=\"" . $h->lang["user_signin_login_form_submit"] . "\" />
									</div>
								</div>
							</form>
							<li class=\"divider\"></li>
							<div style=\"margin:5px 15px 0 15px;\">
								<a href=\"" . BASEURL . "?page=login\" class=\"btn btn-block\">" . $h->lang["user_signin_login_forgot_password"] . "</a>
							</div>
						</ul>
					</li>";
                } else {
    ...
    public function login($h)
    {
        ...
        if (($h->cage->post->testPage('page') == '') || $h->cage->post->keyExists('forgotten_password')) {
