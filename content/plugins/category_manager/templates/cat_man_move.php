...
?>
    ...
    <h2><?php echo $hotaru->lang["cat_man_move"]; ?></h2>

    <table class="cat_man_table">
    <tr><td class="cat_man_body">
        <?php echo $hotaru->lang["cat_man_move_instruct"]; ?> <br /><br />
        <?php
        ...
        ?>
                <div class='cat_move_options' style='display:none'>
                    <form class='cat_man_move_form' style='margin-bottom: 0px;' name='category_manager_move_form' action='<?php echo BASEURL; ?>admin_index.php?page=plugin_settings&amp;plugin=category_manager&amp;action=move_save&amp;id=<?php echo $cat['category_id']; ?>' method='post'>
                            <?php echo $hotaru->lang["cat_man_move_put"]; ?> '<?php echo $cat['category_name']; ?>'   
                            <select name="placement">";
                                <option value="after"><?php echo $hotaru->lang["cat_man_move_after"]; ?></option>
                                <option value="before"><?php echo $hotaru->lang["cat_man_move_before"]; ?></option>
                                <option value="aschild"><?php echo $hotaru->lang["cat_man_move_in"]; ?></option>
                            </select> 
                            <select name="parents">
                            <?php
                            foreach ($hotaru->vars['the_cats'] as $cat2) {
                    ...
                    <?php if ($cat['category_parent'] > 1) { ?>
                        ...
                        <form class='cat_man_move_form' style='margin-bottom: 0px;' name='category_manager_move_form2' action='<?php echo BASEURL; ?>admin_index.php?page=plugin_settings&amp;plugin=category_manager&amp;action=move_save&amp;id=<?php echo $cat['category_id']; ?>' method='post'>
                            <?php echo $hotaru->lang["cat_man_move_move"]; ?>  '<?php echo $cat['category_name']; ?>' 
                            <?php echo $hotaru->lang["cat_man_move_to"]; ?> 
                            <select name='moveup'>
                            <option value='top'><?php echo $hotaru->lang["cat_man_move_top_level"]; ?> </option>
                            <?php
                            foreach ($hotaru->vars['the_cats'] as $cat3) {
