...
<div id="messaging_inbox" class="users_content">

<h2><?php echo $h->lang["messaging_inbox"]; ?></h2>
    
<?php echo $h->showMessages(); ?>

<form name="inbox_form" action="<?php echo BASEURL; ?>index.php?page=inbox" method="post">
<table class="messaging_list">
    <tr class="messaging_list_headers">
        <td class="messaging_fom"><?php echo $h->lang['messaging_from']; ?></td>
    ...
    <?php if (isset($h->vars['messages_list']->list)) { ?>
        ...
        <?php foreach ($h->vars['messages_list']->items as $msg) { ?>
            ...
            <?php $read = (!$msg->message_read) ? "style='font-weight: bold;'" : ""; ?>
                    
            <tr id="<?php echo $msg->message_id; ?>" <?php echo $read; ?>>
