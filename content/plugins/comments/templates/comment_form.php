...
<?php } // JavaScript changes this form! See comments.js ?>
    ...
    <form name='comment_form' action='<?php echo BASEURL; ?>index.php?page=comments' method='post'>
        ...
        <div class="comment_subscribe"><input id="comment_subscribe" name="comment_subscribe" type="checkbox" <?php echo $h->vars['subscribe_check']; ?> /> <?php echo $h->lang['comments_form_subscribe']; ?><?php if ($h->vars['subscribe_check']) { echo " <small>(" . $h->lang['comments_form_unsubscribe'] . ")</small>"; } ?></div>
