...
class Plugin extends generic_pmd
{
    ...
    protected function setPluginFolder($folder)
    {
        $this->folder = $folder;
    }
     ...
     * @return string
     */
    public function get_plugin_status($plugin_folder = '')
    {
        global $db;
    ...
    function install($upgrade = 0)
    {
        ...
        {
            if (version_compare($version, $this->pluginVersion($dependency), '>')) {
                $dependency_error = 1;
            }
        }
        ...
        if ($dependency_error == 1)
        {
            foreach ($this->dependencies as $dependency => $version)
            {
                    if (($this->getPluginStatus($dependency) == 'inactive') 
                        || version_compare($version, $this->pluginVersion($dependency), '>')) {
    ...
    {    
        ...
        $version = $db->get_var($db->prepare("SELECT plugin_version FROM " . TABLE_PLUGINS . " WHERE plugin_folder = %s", $this->folder));
     ...
     */
     function includeCSS($plugin = '', $filename = '')
     {
        ...
        array_push($this->includeCSS, $file_location);
     }
     ...
     */
     function includeJS($plugin = '', $filename = '')
     {
        ...
        // Add this css file to the global array of css_files
        array_push($this->includeJS, $file_location);
     }
}
